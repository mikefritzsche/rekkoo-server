...
-- Refresh maintain_gift_details to handle both INSERT and UPDATE reliably, even when
-- updates come through top-level columns instead of api_metadata/custom_fields.
...
BEGIN
    ...
    IF list_type_val IN ('gift', 'gifts') THEN
        ...
        -- Extract values with fallbacks to top-level columns
        new_quantity := COALESCE((source_data->>'quantity')::integer, NEW.quantity);
        new_rating := COALESCE((source_data->>'rating')::integer, NEW.rating);
