...
-- Improve maintain_gift_details to capture rating updates even when only priority changes are sent.
-- Adds a safe fallback to NEW.priority (which exists on list_items) so rating stays in sync.
...
BEGIN
    ...
    IF list_type_val IN ('gift', 'gifts') THEN
        ...
        -- Extract values (no reliance on missing top-level columns)
        new_quantity := (source_data->>'quantity')::integer;

        -- Rating falls back to priority when no rating is present in metadata
        new_rating := COALESCE((source_data->>'rating')::integer, NEW.priority);
