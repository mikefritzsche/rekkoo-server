...
BEGIN
    ...
    -- Only proceed if it's a gift list
    IF list_type_val = 'gifts' THEN
        
        -- Determine source data: api_metadata > custom_fields
        source_data := COALESCE(NEW.api_metadata, NEW.custom_fields, '{}'::jsonb);
        
        -- Extract values with fallbacks for different casing
        new_quantity := (source_data->>'quantity')::integer;
        new_rating := (source_data->>'rating')::integer;
        ...
        IF NEW.gift_detail_id IS NOT NULL THEN
            ...
            IF NOT FOUND THEN
                -- Re-create it with the existing ID
                INSERT INTO gift_details (
